...
def calc_meteor(path_refs, path_hyp, fld_out='temp', n_lines=None, pretokenized=True):
    ...
    cmd = [
            ...
            '-jar', '/Users/tianhongzxy/meteor-1.5/meteor-1.5.jar', 
...
def calc_nltk_bleu(path_refs, path_hyp, n_lines):
    # return both unsmoothed and smoothed results.
    refs = []
    ...
    for ngram in ngram_weights:
        ...
        for i in range(len(hyps)):
            ...
            _sbleu.append(sentence_bleu(multi_ref, hyps[i].split(), weights=ngram_weights[ngram], smoothing_function=SmoothingFunction().method1))
            #try:
            #	_nist.append(sentence_nist(multi_ref, hyps[i].split(), n=ngram))
            #except ZeroDivisionError:
            #	_nist.append(0.)
        bleu.append(np.mean(_bleu))
        sbleu.append(np.mean(_sbleu))
...
def calc_nlp_metrics(path_refs, path_hyp, fld_out='temp',  n_lines=None):
    ...
    bleu, sbleu = calc_nltk_bleu(path_refs, path_hyp, n_lines)
    #  print('calc_meteor')
    ...
    return bleu, entropy, div, avg_len, sbleu
